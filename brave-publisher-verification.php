...
function well_known_uri($query) {
  ...
  if (is_array($options)) {
    ...
    foreach($options as $key => $value) {
      ...
      header('Content-Type: brave-payments-verification', TRUE);
...
<?php
  ...
  public function page_init() {
    ...
    $suffix_title = 'Path: /.well-known/ (Leave Blank)';
    
    $type_title = 'Content-Type: (Leave Blank)';
...
/*=====================NEED TO REMOVE ADDITIONAL OPTIONS j++====================*/
    else { 
...
/* Need to remove additional uri inputs after verification code */
 ...
 /* these were removed in master but are required for the plugin to work as is, but adding prefilled will go a long way for users */ 
  ...
  public function sanitize_suffix($input, $id) {
        ...
        $valid = array();
        $input[$id] = 'brave-payment-verification.txt';
